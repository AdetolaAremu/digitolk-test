...
class BookingRepository extends BaseRepository
{
    ...
    {
        // since any one hitting this endpoint is a logged in user, I think it is best to use Auth facade.
        $cuser = Auth::user();
        $usertype = '';
        $emergencyJobs = array();
        $noramlJobs = array();
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language:id,lang_name', 'feedback:id,name,phone_no,email')->whereIn('status', ['pending', 'assigned', 'started'])->orderBy('due', 'asc')->get();
            $usertype = $cuser->user_type; // passing the string "customer" is hard coding, while it can come in handy in some cases, we should avoid it when we can get the data from the collection easily.
        } elseif ($cuser && $cuser->is('translator')) {
            $jobs = Job::getTranslatorJobs($cuser->id, 'new');
            $jobs = $jobs->pluck('jobs')->all();
            $usertype = $cuser->user_type; // we should avoid hard coding here
        }
        if ($jobs) {
     ...
     * @return array
     */
    public function getUsersJobsHistory(Request $request)
    {
        ...
        $noramlJobs = array();
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language:id,lang_name', 'feedback:id,name,phone_no,email', 'distance:id,longitude,latitude')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due', 'desc')->paginate(15);
            $usertype = $cuser->user_type; // we should avoid hard coding here
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type === env('CUSTOMER_ROLE_ID')) { // to make sure the operator is strict ===
            $cuser = $user;
            ...
            if ($data['immediate'] === 'no') { // to make sure the operator is strict ===
    ...
    public function endJob($post_data)
    {
        $completeddate = date('Y-m-d H:i:s');
        $jobid = $post_data["job_id"];
        $job_detail = Job::with('translatorJobRel:id,language,certified,gender')->find($jobid);
    ...
    public function bookingExpireNoAccepted()
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            $allJobs = User::join('languages', 'jobs.from_language_id', '=', 'languages.id')
